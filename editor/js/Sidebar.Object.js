...
// ---------------- appTangible ----------------
import { interfazProg } from '../appTangible/interfazProg.js'
import { receptorBLE } from '../appTangible/receptorBLE.js';
...
function SidebarObject( editor ) {
	...
	*/
	...
	// SensorsMenu --> appTangible

	var objectSensorsMenuRow = new UIRow();
	var objectSensorsMenu = new UIText();
	var objectSensorsMenuButton = new UIButton( strings.getKey( 'sidebar/object/SensorsMenuButton' ) ).onClick( function () {

		if(receptorBLE.getConnected()){
			const menuSensores = document.querySelector("#menu");
			menuSensores.style.display = 'block';
			interfazProg.importFigure();
		}else{
			alert("Please, connect a Bluetooth device first.")
		}
	...
	objectSensorsMenuRow.add( new UIText( strings.getKey( 'sidebar/object/SensorsMenu' ) ).setWidth( '90px' ) );
	objectSensorsMenuRow.add( objectSensorsMenu );
	objectSensorsMenuRow.add( objectSensorsMenuButton );

	container.add( objectSensorsMenuRow );
